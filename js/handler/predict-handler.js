async function predictImpactOfAllCcqAtCurrentDay(listAllCcqInfor) {
  ...
  for (let ccqInfor of listAllCcqInfor) {
    if (
      ccqInfor.fundAssetType === FUND_TYPE_STOCK ||
      ccqInfor.fundAssetType === FUND_TYPE_BALANCED
    ) {
      let ccqDetailInfor = await handleDataDetailCcq(ccqInfor.shortName);
      let predictImpactAtCurrentDay = predictPriceOfStockOrBalancedCcq(
        ccqDetailInfor,
        false
      );
      predictImpactAtCurrentDay =
        Math.round(predictImpactAtCurrentDay * 10000) / 100;
...
function predictPriceOfStockOrBalancedCcq(ccqInfor, isGetOldData) {
  ...
  if (isWorkingDay(new Date())) {
    ...
    let impactStockPercent =
      predictImpactPercentStockComponent(listStockComponent);
    ...
    if (
      ...
      for (let assetPercent of listAllAssetPercent) {
        if (assetPercent.code == FUND_TYPE_STOCK) {
          realAllStockPercent = assetPercent.percent;
          break;
        }
      }
      predictImpactPercentList.push(
        Math.round(
          (impactStockPercent / allStockComponentPercent) *
            realAllStockPercent *
            100
        ) / 100
      );
  ...
  if (isGetOldData) {
    let previousWorkingDateInStringFormat = formatDate(
      getPreviousWorkingDay(new Date())
    );
    // console.log("Previous Working Date: "+previousWorkingDateInStringFormat);
    // console.log("Ccq Infor Date: "+ccqInfor.curNavDate);
    ...
    while (ccqInfor.curNavDate <= previousWorkingDateInStringFormat) {
      let previousDateInDateFormat = new Date(
        previousWorkingDateInStringFormat
      );
      ...
      // console.log("Vào chức năng");
      predictImpactPercentList.push(
        getPreviousPredictValueByCcqShortName(
          ccqInfor.shortName,
          previousWorkingDateInStringFormat
        )
      );
      previousWorkingDateInStringFormat = formatDate(
        getPreviousWorkingDay(previousDateInDateFormat)
      );
    }
  }
  // console.log(predictImpactPercentList);
...
function predictImpactPercentStockComponent(
  listStockComponent,
  isStockMarketInBigImpact
) {
  let result = 0;
  // let listAllMultiple = [];
...
function getPreviousPredictValueByCcqShortName(
  ccqShortName,
  selectedDateInStringFormat
) {
  let predictValue = 0;
  let keyPredictImpactPreviousDay =
    CONSTANT_INFER_LASTED_IMPACT_OF_PREVIOUS_DAY + selectedDateInStringFormat;
  let mapPredictImpactCcqOfPreviousDay = retrieveDataFromLocalStorage(
    keyPredictImpactPreviousDay
  );
  // console.log(mapPredictImpactCcqOfPreviousDay);
  if (
    mapPredictImpactCcqOfPreviousDay &&
    mapPredictImpactCcqOfPreviousDay != null
  ) {
...
function calcAllCaseOfListImpactPercentOfEachComponent(
  generalArray,
  currentRow,
  previousMultipleResult,
  listResult
) {
  for (let number of generalArray[currentRow]) {
    ...
    if (currentRow < generalArray.length - 1) {
      calcAllCaseOfListImpactPercentOfEachComponent(
        generalArray,
        currentRow + 1,
        multipleResult,
        listResult
...
function getImpactPercentOfStockComponent(
  order,
  isStockMarketInBigImpact,
  updateAt
) {
  ...
  if (order < 3) {
    ...
    impactPercentDefault = PRICE_MOVEMENT_FOR_TOP_3_STOCK_COMPONENT;
  } else {
    impactPercentDefault = PRICE_MOVEMENT_FOR_TOP_4_TO_N_STOCK_COMPONENT;
  }
  impactPercentDefault =
    impactPercentDefault * (currentMonth - componentMonth + 1);
  if (isStockMarketInBigImpact === true) {
    return impactPercentDefault * RATIO_FOR_BIG_STOCK_IMPACT;
  } else {
