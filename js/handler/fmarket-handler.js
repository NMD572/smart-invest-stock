...
async function handleDataDetailCcq(ccqShortName) {
  ...
  let listFundAssetTypeNeedToCompare =
    getListFundAssetTypeNeedToCompare(currentFundAssetType);
  ...
  let result = new CcqInforToLoadDetail(
    originalCcqData.data.id,
    originalCcqData.data.shortName,
    originalCcqData.data.code,
    originalCcqData.data.name,
    originalCcqData.data.owner.shortName,
    currentFundAssetType,
    originalCcqData.data.contentHome && originalCcqData.data.contentHome != null
      ? originalCcqData.data.contentHome.shortDesc
      : "",
    originalCcqData.data.extra.currentNAV,
    convertLongToDateFormat(originalCcqData.data.extra.lastNAVDate),
    originalCcqData.data.productTradingSession.closedBankNoteTimeString,
    originalCcqData.data.productTradingSession.tradingTimeString,
    listStockOfCcq,
    listInvestGroupPercent,
    listAssetPercent,
    listFundAssetTypeNeedToCompare,
    "-",
    originalCcqData.data.productTransactionDateModelList
  );
  return result;
}
...
function getListAssetPercent(originalCcqData) {
  ...
  if (originalCcqData.data.productAssetHoldingList) {
    for (
      let i = 0, end = originalCcqData.data.productAssetHoldingList.length;
      i < end;
      ++i
    ) {
      let singleData = originalCcqData.data.productAssetHoldingList[i];
      let assetPercentData = new AssetPercentOfCcq(
        singleData.assetType.code,
        singleData.assetType.name,
        singleData.assetPercent
      );
...
function getListInvestGroup(originalCcqData) {
  ...
  if (originalCcqData.data.productIndustriesHoldingList) {
    for (
      let i = 0, end = originalCcqData.data.productIndustriesHoldingList.length;
      i < end;
      ++i
    ) {
      let singleData = originalCcqData.data.productIndustriesHoldingList[i];
      let investGroupData = new InvestGroupPercentOfCcq(
        singleData.industry,
        singleData.assetPercent
      );
...
function getAndConvertDataListStockOfCcq(originalCcqData) {
  ...
  if (originalCcqData.data.productTopHoldingList) {
    for (
      let i = 0, end = originalCcqData.data.productTopHoldingList.length;
      i < end;
      ++i
    ) {
      let singleData = originalCcqData.data.productTopHoldingList[i];
      let stockData = new InvestComponentDetailData(
        singleData.stockCode,
        singleData.industry,
        singleData.netAssetPercent,
        singleData.price,
        singleData.changeFromPrevious,
        singleData.changeFromPreviousPercent,
        FUND_TYPE_STOCK,
        singleData.updateAt
      );
  ...
  if (originalCcqData.data.productTopHoldingBondList) {
    for (
      let i = 0, end = originalCcqData.data.productTopHoldingBondList.length;
      i < end;
      ++i
    ) {
...
function getListFundAssetTypeNeedToCompare(fundAssetType) {
  let listFundAssetTypeNeedToCompare = [];
  if (fundAssetType == FUND_TYPE_BOND) {
    // Is BOND CCQ --> list compare: list all BOND CCQ
    listFundAssetTypeNeedToCompare.push(FUND_TYPE_BOND);
  } else {
    // Is not BOND CCQ --> list compare: list STOCK CCQ and BALANCE CCQ
    listFundAssetTypeNeedToCompare.push(FUND_TYPE_STOCK);
    listFundAssetTypeNeedToCompare.push(FUND_TYPE_BALANCED);
  }
...
async function getListCcqInfor(listFundAssetTypeNeedToCompare) {
  ...
  for (
    let interatorFundType = 0,
      sizeFundType = listFundAssetTypeNeedToCompare.length;
    interatorFundType < sizeFundType;
    ++interatorFundType
  ) {
    let listCcq = await callApiGetListInvestementCertificateSTOCK(
      SORT_FIELD_YTD,
      listFundAssetTypeNeedToCompare[interatorFundType]
    );
    for (let i = 0, end = listCcq.data.total; i < end; ++i) {
      ...
      if (listCcq.data.rows[i].isProductIpo == false) {
        let ccq = new CCQInfor(
          listCcq.data.rows[i].id,
          listCcq.data.rows[i].shortName,
...
async function getLastedNavOfCcqFromDataDateToPreviousDate(ccqId, dataDate) {
  ...
  previous30DayDate = convertLongToDateFormat(
    previous30DayDate.setDate(previous30DayDate.getDate() - 30)
  );
  jsonDatas = await callApiGetListNavHistoryOfCcq(
    ccqId,
    convertDateInputToDateFormatOfFmarket(previous30DayDate),
    convertDateInputToDateFormatOfFmarket(dataDate),
    isGetAll
  );
...
async function getListNavHistory(ccqId, fromDate, toDate, isGetAll, chartType) {
  let jsonDatas = await callApiGetListNavHistoryOfCcq(
    ccqId,
    convertDateInputToDateFormatOfFmarket(fromDate),
    convertDateInputToDateFormatOfFmarket(toDate),
    isGetAll
  );
  ...
  if (chartType == getChartTypeCurrencyVND()) {
    for (let i = 0, end = jsonDatas.data.length; i < end; ++i) {
      ...
      if (i > 0) {
        growthFromPreviousDay = calculateGrowthRatioFromPreviousDay(
          jsonDatas.data[i - 1].nav,
          jsonDatas.data[i].nav
        );
      }
      listNavHistoryInfor.push(
        new NavCcqHistory(
          jsonDatas.data[i].nav,
          jsonDatas.data[i].navDate,
    ...
    for (let i = 0, end = jsonDatas.data.length; i < end; ++i) {
      ...
      listNavHistoryInfor.push(
        new NavCcqHistory(
          calculateGrowthRatio(firstDayPrice, jsonDatas.data[i].nav),
          jsonDatas.data[i].navDate,
          growthFromPreviousDay
        )
      );
